...
export class TaskService {
  ...
  getTasksForUser(userId: number, requesterId?: number): Observable<Task[]> {
    const url = requesterId
      ? `${this.baseUrl}/user/${userId}?requesterId=${requesterId}`
  ...
  completeTask(taskId: number, requesterId?: number): Observable<Task> {
    const url = requesterId
      ? `${this.baseUrl}/${taskId}/complete?requesterId=${requesterId}`
  ...
  uncompleteTask(taskId: number, requesterId?: number): Observable<Task> {
    const url = requesterId
      ? `${this.baseUrl}/${taskId}/uncomplete?requesterId=${requesterId}`
      : `${this.baseUrl}/${taskId}/uncomplete`;
    return this.http.put<Task>(url, {});
  }

  getTasksForFamily(familyEmail: string, requesterId?: number): Observable<Task[]> {
    const url = requesterId
      ? `${this.baseUrl}/family/${familyEmail}?requesterId=${requesterId}`
      : `${this.baseUrl}/family/${familyEmail}`;
    return this.http.get<Task[]>(url);
  }
}
